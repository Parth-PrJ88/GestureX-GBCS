...
# Updated: brings camera windows to front, fixes heatmap broadcast bug,
# adds snap-on-large-jump for cursor smoothing, hides OS cursor during fullscreen calibration.
# added Cursor hide/show functions for Windows using ctypes.
# Requirements:
# pip install mediapipe opencv-python numpy pyautogui scipy PyGetWindow
# pip install pip install pywin32 (Windows) for window focus or pyobjc-core pyobjc (Mac) for Mac
...
def heatmap_add(px, py, strength=1.0, sigma=10.0):
    ...
    # Correct orientation: dx (columns) and dy (rows)
    dx = np.arange(x0, x1 + 1) - cx   # length = (x1-x0+1)
    dy = np.arange(y0, y1 + 1) - cy   # length = (y1-y0+1)

    # meshgrid(dx, dy) -> X shape (len(dy), len(dx)), Y same
    X, Y = np.meshgrid(dx, dy)
    gauss = np.exp(- (X.astype(np.float32)**2 + Y.astype(np.float32)**2) / (2.0 * (sigma**2)))

    # Now add: heatmap slice shape = (y1-y0+1, x1-x0+1) matches gauss
...
# ----------------------
# Calibration flow (same as before, hides system cursor)
# ----------------------
def mode_calibration(num_points=9):
    ...
    # bring to front after a quick initial show
    canvas_show = make_fullscreen_canvas(255)
    cv2.imshow(win, canvas_show); cv2.waitKey(1)
    bring_window_to_front(win)
    hide_system_cursor()   # hide OS cursor during fullscreen calibration
    ...
    for i, (tx, ty) in enumerate(targets):
        ...
        steps = max(1, int(TRANSITION_SEC * 60))
    ...
    try:
        while True:
            ...
            if centers:
                ...
                # if first sample, set ema directly
                if ema_x is None:
                    ema_x, ema_y = px, py
                else:
                    # distance between predicted and current ema
                    dist = math.hypot(px - ema_x, py - ema_y)
                    if dist > JUMP_RESET_PX:
                        # big jump -> snap immediately (makes it snappy)
                        ema_x, ema_y = px, py
                    else:
                        # dynamic alpha: bigger movement -> higher alpha (more responsive)
            ...
            if ema_x is not None:
                # draw semi-transparent big circle; draw outline
                overlay = canvas.copy()
        ...
        show_cursor()
        print("Exited cursor control mode.")
...
if __name__ == "__main__":
    print("""
    =======================================================
    GAZE PROTOTYPE MENU OPTIONS:
    =======================================================
    1 -> Alignment (Preview camera + Alignment reference)
