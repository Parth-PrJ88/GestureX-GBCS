# src/gaze_prototype_with_alignment_4x4_with_drags_fixed_endpoint.py
# Requirements:
# pip install mediapipe opencv-python numpy pyautogui scipy PyGetWindow keyboard
#
# Save alignment reference image (optional) at: assets/align_reference.png

import sys, time, math, ctypes
import threading
from pathlib import Path
import json
import cv2
import mediapipe as mp
import numpy as np
import pyautogui
import csv
from scipy.stats import pearsonr
import keyboard

try:
    import pygetwindow as gw
except Exception:
    gw = None

# ----------------------
# Configuration & paths
# ----------------------
OUT_CSV = Path("data/eye_landmarks.csv")
OUT_MAP = Path("data/gaze_map.json")
OUT_DRAG_JSON = Path("data/drag_samples.json")
PROFILES_DIR = Path("data/profiles")

OUT_CSV.parent.mkdir(parents=True, exist_ok=True)
OUT_DRAG_JSON.parent.mkdir(parents=True, exist_ok=True)
PROFILES_DIR.mkdir(parents=True, exist_ok=True)

ALIGN_IMAGE_PATH = Path("assets/align_reference.png")

# Calibration params
SAMPLES_PER_POINT = 30
SAMPLES_PER_DRAG = 30
PRE_FIXATION_SEC = 0.8
TRANSITION_SEC = 0.8
DRAG_TRANSITION_SEC = 2.2
GRID_POINTS = 16
TARGET_COLOR = (0, 0, 255)
BACKGROUND_BRIGHTNESS = 255
TARGET_RADIUS = 30

# Camera preview
CAM_PREVIEW_SIZE = (360, 270)
PREVIEW_MARGIN = 20

# Face & lighting checks
MIN_FACE_AREA_PX = 3000
MIN_FACE_BRIGHTNESS = 60

# Cursor smoothing & heatmap
EMA_ALPHA = 0.18
MOVE_THRESHOLD_PX = 5
VIRTUAL_CURSOR_RADIUS = 60
HEATMAP_DECAY = 0.95
HEATMAP_SCALE = 4

# Jump behaviour
JUMP_RESET_PX = 200

# Screen size
SCREEN_W, SCREEN_H = pyautogui.size()

# Wink / gesture parameters
EYE_ASPECT_RATIO_THRESHOLD = 0.22
WINK_MIN_DURATION_MS = 1500  # 1.5 seconds required for a wink to count as click
WINK_COOLDOWN_MS = 800      # cooldown between repeated wink clicks
DOUBLE_CLICK_HOLD_MS = 800  # time holding left wink required to trigger a double click

# Wink robustness: smoothing + hysteresis to prevent corner/edge failures
# EAR EMA smoothing: blends raw EAR with previous smoothed value each frame.
# Lower = more stable (less reactive to flicker), higher = faster response.
EAR_EMA_ALPHA = 0.35
# Number of consecutive "eye open" frames required before resetting the wink
# state. Prevents a single noisy landmark frame from interrupting a hold.
WINK_RESET_FRAMES = 5

# Pinch hold robustness: smoothing + hysteresis so detection dropouts and
# jittery finger distance readings don't release the hold unexpectedly.
PINCH_DIST_EMA_ALPHA = 0.4   # distance smoothing (higher = faster, lower = smoother)
PINCH_RELEASE_FRAMES = 8     # consecutive non-pinch frames needed to actually release

# Global state for active profile tracking
ACTIVE_PROFILE_NAME = "default"

# ----------------------
# MediaPipe init
# ----------------------
mp_face = mp.solutions.face_mesh
face_mesh = mp_face.FaceMesh(max_num_faces=1,
                            refine_landmarks=True,
                            min_detection_confidence=0.5,
                            min_tracking_confidence=0.5)

# Initialize Hand tracking for Drag functionality
mp_hands = mp.solutions.hands
hands = mp_hands.Hands(max_num_hands=1,
                       min_detection_confidence=0.7,
                       min_tracking_confidence=0.7)
mp_drawing = mp.solutions.drawing_utils

LEFT_IRIS = [468, 469, 470, 471, 472]
RIGHT_IRIS = [473, 474, 475, 476, 477]

# ----------------------
# Helpers
# ----------------------
def avg_landmark(landmarks, idxs):
    pts = [(landmarks[i].x, landmarks[i].y) for i in idxs]
    xs = [p[0] for p in pts]; ys = [p[1] for p in pts]
    return float(np.mean(xs)), float(np.mean(ys))

def get_iris_centers(results):
    if not results.multi_face_landmarks:
        return None
    lm = results.multi_face_landmarks[0].landmark
    lx, ly = avg_landmark(lm, LEFT_IRIS)
    rx, ry = avg_landmark(lm, RIGHT_IRIS)
    return lx, ly, rx, ry

def calculate_ear(landmarks, indices):
    p_left = np.array([landmarks[indices[0]].x, landmarks[indices[0]].y])
    p_top = np.array([landmarks[indices[1]].x, landmarks[indices[1]].y])
    p_bottom = np.array([landmarks[indices[2]].x, landmarks[indices[2]].y])
    p_right = np.array([landmarks[indices[3]].x, landmarks[indices[3]].y])

    v_dist = np.linalg.norm(p_top - p_bottom)
    h_dist = np.linalg.norm(p_left - p_right)

    if h_dist == 0: return 0.0
    return v_dist / h_dist

def get_face_bbox_pixels(results, frame_w, frame_h):
    if not results.multi_face_landmarks:
        return None
    lm = results.multi_face_landmarks[0].landmark
    xs = [p.x for p in lm]; ys = [p.y for p in lm]
    minx = int(min(xs) * frame_w); maxx = int(max(xs) * frame_w)
    miny = int(min(ys) * frame_h); maxy = int(max(ys) * frame_h)
    minx = max(0, minx); miny = max(0, miny)
    maxx = min(frame_w-1, maxx); maxy = min(frame_h-1, maxy)
    return (minx, miny, maxx, maxy)

def face_present_and_bright(results, frame, min_area_px=MIN_FACE_AREA_PX, min_brightness=MIN_FACE_BRIGHTNESS):
    if not results.multi_face_landmarks:
        return False, 0.0, None
    h, w = frame.shape[:2]
    bbox = get_face_bbox_pixels(results, w, h)
    if bbox is None:
        return False, 0.0, None
    minx, miny, maxx, maxy = bbox
    area = (maxx - minx) * (maxy - miny)
    face_crop = frame[miny:maxy, minx:maxx]
    if face_crop.size == 0:
        return False, 0.0, bbox
    gray = cv2.cvtColor(face_crop, cv2.COLOR_BGR2GRAY)
    mean_brightness = float(np.mean(gray))
    present = (area >= min_area_px) and (mean_brightness >= min_brightness)
    return present, mean_brightness, bbox

# ----------------------
# Window helpers
# ----------------------
def bring_window_to_front_windows(title, retries=8, delay=0.08):
    try:
        user32 = ctypes.windll.user32
        SWP_NOSIZE = 0x0001
        SWP_NOMOVE = 0x0002
        HWND_TOPMOST = -1
        HWND_NOTOPMOST = -2
        for _ in range(retries):
            hwnd = user32.FindWindowW(None, title)
            if hwnd:
                user32.SetWindowPos(hwnd, HWND_TOPMOST, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE)
                time.sleep(0.03)
                user32.SetWindowPos(hwnd, HWND_NOTOPMOST, 0,0,0,0, SWP_NOMOVE | SWP_NOSIZE)
                return True
            time.sleep(delay)
    except Exception:
        return False
    return False

def bring_window_pygetwindow(title, retries=8, delay=0.08):
    if gw is None:
        return False
    try:
        for _ in range(retries):
            wins = gw.getWindowsWithTitle(title)
            if wins:
                try:
                    wins[0].activate()
                    return True
                except Exception:
                    pass
            time.sleep(delay)
    except Exception:
        return False
    return False

def bring_window_to_front(title):
    if sys.platform.startswith("win"):
        if bring_window_to_front_windows(title):
            return True
    return bring_window_pygetwindow(title)

def hide_system_cursor():
    if sys.platform.startswith("win"):
        ctypes.windll.user32.ShowCursor(False)

def show_system_cursor():
    if sys.platform.startswith("win"):
        ctypes.windll.user32.ShowCursor(True)

# ----------------------
# Mapping & Profile Profile helpers
# ----------------------
def build_design_matrix(eye_xy):
    x = eye_xy[:,0]; y = eye_xy[:,1]
    return np.column_stack([x, y, x*y, x**2, y**2, np.ones_like(x)])

def fit_poly_mapping(data):
    arr = np.array(data)
    eye = arr[:,0:2]; sx = arr[:,2]; sy = arr[:,3]
    Phi = build_design_matrix(eye)
    wx, *_ = np.linalg.lstsq(Phi, sx, rcond=None)
    wy, *_ = np.linalg.lstsq(Phi, sy, rcond=None)
    return wx, wy

def predict_poly(wx, wy, ex, ey):
    f = np.array([ex, ey, ex*ey, ex*ex, ey*ey, 1.0])
    px = float(np.dot(f, wx))
    py = float(np.dot(f, wy))
    return px, py

def save_mapping(wx, wy, invert_x=False, invert_y=False, profile_name="default"):
    mapping = {
        "profile_name": profile_name,
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "wx": wx.tolist(), 
        "wy": wy.tolist(),
        "screen": [SCREEN_W, SCREEN_H],
        "invert_x": bool(invert_x), 
        "invert_y": bool(invert_y)
    }
    
    with open(OUT_MAP, "w") as f:
        json.dump(mapping, f, indent=4)
        
    safe_filename = "".join([c for c in profile_name if c.isalpha() or c.isdigit() or c in (' ', '_', '-')]).rstrip()
    safe_filename = safe_filename.replace(" ", "_").lower()
    if not safe_filename:
        safe_filename = "unnamed_profile"
        
    profile_path = PROFILES_DIR / f"{safe_filename}.json"
    with open(profile_path, "w") as f:
        json.dump(mapping, f, indent=4)
    print(f"\n[SUCCESS] Calibration saved successfully under node: '{profile_name}'")

def load_mapping(profile_filename=None):
    if profile_filename:
        target_path = PROFILES_DIR / f"{profile_filename}.json"
        if target_path.exists():
            return json.load(open(target_path))
    
    if OUT_MAP.exists():
        return json.load(open(OUT_MAP))
    return None

def interactive_profile_menu():
    global ACTIVE_PROFILE_NAME
    while True:
        print("\n=== CALIBRATION NODE PROFILE MANAGEMENT ===")
        profiles = list(PROFILES_DIR.glob("*.json"))
        
        current_loaded = "None"
        if OUT_MAP.exists():
            try:
                with open(OUT_MAP, "r") as f:
                    data = json.load(f)
                    current_loaded = f"{data.get('profile_name', 'default')} ({data.get('timestamp', 'Unknown')})"
            except Exception:
                current_loaded = "gaze_map.json (Active cache)"

        print(f"Current System Active Profile: {current_loaded}")
        print("-" * 43)
        
        if not profiles:
            print("No saved profile calibration nodes found inside 'data/profiles/'.")
        else:
            print("Available nodes:")
            for i, p_path in enumerate(profiles, start=1):
                try:
                    with open(p_path, "r") as f:
                        meta = json.load(f)
                    print(f"  {i} -> {meta.get('profile_name', p_path.stem)} [{meta.get('timestamp', '')}]")
                except Exception:
                    print(f"  {i} -> {p_path.stem} (Raw JSON Mapping Error)")

        print("\nOptions:")
        print("  [number] -> Load selected profile node configuration")
        print("  m        -> Return to Master Menu")
        
        choice = input("\nSelect an action: ").strip()
        if choice.lower() == 'm':
            break
            
        if choice.isdigit():
            idx = int(choice) - 1
            if 0 <= idx < len(profiles):
                selected_profile = profiles[idx]
                try:
                    with open(selected_profile, "r") as sf:
                        profile_data = json.load(sf)
                    with open(OUT_MAP, "w") as df:
                        json.dump(profile_data, df, indent=4)
                    
                    ACTIVE_PROFILE_NAME = profile_data.get("profile_name", selected_profile.stem)
                    print(f"\n[LOADED] System active profile switched to: '{ACTIVE_PROFILE_NAME}'")
                    break
                except Exception as e:
                    print(f"Error swapping node profile content maps: {e}")
            else:
                print("Index out of selection range.")
        else:
            print("Invalid Option selection.")

# ----------------------
# Heatmap helpers
# ----------------------
heatmap_h = max(1, SCREEN_H // HEATMAP_SCALE)
heatmap_w = max(1, SCREEN_W // HEATMAP_SCALE)
heatmap = np.zeros((heatmap_h, heatmap_w), dtype=np.float32)

def heatmap_add(px, py, strength=1.0, sigma=10.0):
    global heatmap, heatmap_w, heatmap_h
    cx = int(round(px / HEATMAP_SCALE))
    cy = int(round(py / HEATMAP_SCALE))
    if cx < 0 or cx >= heatmap_w or cy < 0 or cy >= heatmap_h:
        return
    sigma = max(1.0, float(sigma))
    half_size = int(min(max(3 * sigma, 3), max(heatmap_w, heatmap_h)))
    x0 = max(0, cx - half_size)
    x1 = min(heatmap_w - 1, cx + half_size)
    y0 = max(0, cy - half_size)
    y1 = min(heatmap_h - 1, cy + half_size)
    dx = np.arange(x0, x1 + 1) - cx
    dy = np.arange(y0, y1 + 1) - cy
    X, Y = np.meshgrid(dx, dy)
    gauss = np.exp(- (X.astype(np.float32)**2 + Y.astype(np.float32)**2) / (2.0 * (sigma**2)))
    heatmap[y0:y1+1, x0:x1+1] += (strength * gauss)
    np.clip(heatmap, 0.0, None, out=heatmap)

def heatmap_decay():
    global heatmap
    heatmap *= HEATMAP_DECAY
    np.clip(heatmap, 0.0, None, out=heatmap)

def heatmap_render():
    hm = heatmap.copy()
    if hm.max() > 0:
        hm = hm / (hm.max()) * 255.0
    hm = np.clip(hm, 0, 255).astype(np.uint8)
    hm_color = cv2.applyColorMap(cv2.resize(hm, (SCREEN_W, SCREEN_H)), cv2.COLORMAP_JET)
    alpha = cv2.resize(hm, (SCREEN_W, SCREEN_H)).astype(np.float32) / 255.0
    alpha = np.clip(alpha, 0, 1)
    overlay = (hm_color.astype(np.float32) * alpha[..., None]).astype(np.uint8)
    return overlay

# ----------------------
# UI helpers
# ----------------------
def make_fullscreen_canvas(color=BACKGROUND_BRIGHTNESS):
    return np.full((SCREEN_H, SCREEN_W, 3), color, dtype=np.uint8)

# ----------------------
# Small shared design system for the calibration / alignment screens.
#
# Colors are BGR (OpenCV convention). The canvas itself is deliberately
# kept bright/near-white rather than switched to a dark theme: a bright
# screen helps illuminate the user's face for iris tracking, and that
# behaviour is left untouched. Everything below is purely cosmetic —
# no timing, thresholds, sample counts, or control flow are affected.
# ----------------------
UI_PANEL        = (255, 255, 255)   # card / panel fill
UI_PANEL_SHADOW = (210, 210, 210)   # soft shadow under cards
UI_TEXT_DARK    = (45, 45, 45)
UI_TEXT_MUTED   = (135, 135, 135)
UI_ACCENT       = (235, 149, 52)    # calm blue accent
UI_SUCCESS      = (100, 181, 68)    # green
UI_ERROR        = (72, 72, 235)     # red
UI_TARGET       = (66, 66, 231)     # softer "flat" red for the fixation dot
UI_TARGET_RING  = (235, 149, 52)    # accent glow ring around the target


def _blend(canvas, overlay, alpha):
    """Alpha-blend `overlay` onto `canvas` in place (for soft shadows / glows)."""
    cv2.addWeighted(overlay, alpha, canvas, 1 - alpha, 0, dst=canvas)


def draw_rounded_rect(canvas, pt1, pt2, radius, color, thickness=-1, line_type=cv2.LINE_AA):
    """Filled (thickness=-1) or outlined rounded rectangle."""
    x1, y1 = pt1
    x2, y2 = pt2
    radius = max(0, min(radius, (x2 - x1) // 2, (y2 - y1) // 2))
    if thickness < 0:
        cv2.rectangle(canvas, (x1 + radius, y1), (x2 - radius, y2), color, -1, line_type)
        cv2.rectangle(canvas, (x1, y1 + radius), (x2, y2 - radius), color, -1, line_type)
        for cx, cy in ((x1 + radius, y1 + radius), (x2 - radius, y1 + radius),
                       (x1 + radius, y2 - radius), (x2 - radius, y2 - radius)):
            cv2.circle(canvas, (cx, cy), radius, color, -1, line_type)
    else:
        cv2.line(canvas, (x1 + radius, y1), (x2 - radius, y1), color, thickness, line_type)
        cv2.line(canvas, (x1 + radius, y2), (x2 - radius, y2), color, thickness, line_type)
        cv2.line(canvas, (x1, y1 + radius), (x1, y2 - radius), color, thickness, line_type)
        cv2.line(canvas, (x2, y1 + radius), (x2, y2 - radius), color, thickness, line_type)
        cv2.ellipse(canvas, (x1 + radius, y1 + radius), (radius, radius), 180, 0, 90, color, thickness, line_type)
        cv2.ellipse(canvas, (x2 - radius, y1 + radius), (radius, radius), 270, 0, 90, color, thickness, line_type)
        cv2.ellipse(canvas, (x1 + radius, y2 - radius), (radius, radius), 90, 0, 90, color, thickness, line_type)
        cv2.ellipse(canvas, (x2 - radius, y2 - radius), (radius, radius), 0, 0, 90, color, thickness, line_type)


def put_text_shadowed(canvas, text, org, font, scale, color, thickness,
                       shadow_color=(0, 0, 0), shadow_offset=2, line_type=cv2.LINE_AA):
    """Text with a soft dark shadow so it stays readable on the bright background."""
    x, y = org
    cv2.putText(canvas, text, (x + shadow_offset, y + shadow_offset), font, scale,
                shadow_color, thickness + 1, line_type)
    cv2.putText(canvas, text, (x, y), font, scale, color, thickness, line_type)


def text_size(text, font, scale, thickness):
    return cv2.getTextSize(text, font, scale, thickness)[0]


def draw_status_banner(canvas, message, progress=None, kind="info"):
    """
    Rounded status banner pinned to the bottom-center of the screen.
    kind: 'info' (blue), 'success' (green), 'error' (red)
    progress: optional (current, total) tuple to render a slim progress bar.
    """
    color = {"info": UI_ACCENT, "success": UI_SUCCESS, "error": UI_ERROR}.get(kind, UI_ACCENT)
    font = cv2.FONT_HERSHEY_SIMPLEX
    scale, thickness = 0.72, 2
    tw, th = text_size(message, font, scale, thickness)

    panel_w = max(tw + 90, 360)
    panel_h = 78 if progress is not None else 60
    x0 = (SCREEN_W - panel_w) // 2
    y1 = SCREEN_H - 34
    y0 = y1 - panel_h

    shadow = canvas.copy()
    draw_rounded_rect(shadow, (x0, y0 + 5), (x0 + panel_w, y1 + 5), 16, UI_PANEL_SHADOW, -1)
    _blend(canvas, shadow, 0.4)

    draw_rounded_rect(canvas, (x0, y0), (x0 + panel_w, y1), 16, UI_PANEL, -1)
    draw_rounded_rect(canvas, (x0, y0), (x0 + panel_w, y1), 16, color, 2)

    dot_cx, dot_cy = x0 + 26, y0 + 30
    cv2.circle(canvas, (dot_cx, dot_cy), 6, color, -1, cv2.LINE_AA)

    tx = dot_cx + 20
    ty = dot_cy + th // 2
    cv2.putText(canvas, message, (tx, ty), font, scale, UI_TEXT_DARK, thickness, cv2.LINE_AA)

    if progress is not None:
        cur, total = progress
        total = max(1, total)
        frac = max(0.0, min(1.0, cur / total))
        bx0, by0 = x0 + 26, y0 + panel_h - 22
        bx1, by1 = x0 + panel_w - 26, y0 + panel_h - 14
        draw_rounded_rect(canvas, (bx0, by0), (bx1, by1), 4, (228, 228, 228), -1)
        fill_x1 = bx0 + int((bx1 - bx0) * frac)
        if fill_x1 > bx0:
            draw_rounded_rect(canvas, (bx0, by0), (fill_x1, by1), 4, color, -1)


def draw_camera_inset(canvas, cam_frame, ok=True, label="Camera preview"):
    """Small framed live camera preview in the corner, with a status-colored border."""
    small = cv2.resize(cam_frame, CAM_PREVIEW_SIZE)
    h, w = CAM_PREVIEW_SIZE[1], CAM_PREVIEW_SIZE[0]
    x0, y0 = PREVIEW_MARGIN, PREVIEW_MARGIN
    border_color = UI_SUCCESS if ok else UI_ERROR

    shadow = canvas.copy()
    draw_rounded_rect(shadow, (x0 - 4, y0), (x0 + w + 4, y0 + h + 8), 10, UI_PANEL_SHADOW, -1)
    _blend(canvas, shadow, 0.35)

    draw_rounded_rect(canvas, (x0 - 4, y0 - 4), (x0 + w + 4, y0 + h + 4), 10, (255, 255, 255), -1)
    canvas[y0:y0 + h, x0:x0 + w] = small
    draw_rounded_rect(canvas, (x0 - 4, y0 - 4), (x0 + w + 4, y0 + h + 4), 10, border_color, 2)

    chip_y = y0 + h + 22
    cv2.circle(canvas, (x0 + 8, chip_y - 4), 5, border_color, -1, cv2.LINE_AA)
    cv2.putText(canvas, label, (x0 + 20, chip_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, UI_TEXT_MUTED, 1, cv2.LINE_AA)


def countdown_on_canvas(win_name, seconds=3, cam=None):
    """
    Smooth, continuously-rendered countdown.

    The previous version drew one static digit, then called time.sleep(1)
    -- the screen (and the camera preview inside it) simply froze for a
    full second before popping to the next number. This version redraws
    every frame for the whole countdown (same total duration = `seconds`,
    same digits shown: seconds..1), so the camera feed stays live and each
    digit eases in with a small pop + a thin progress ring, instead of
    hard-cutting between frozen frames.
    """
    font = cv2.FONT_HERSHEY_DUPLEX
    total = float(seconds)
    t_start = time.time()
    last_frame = None

    while True:
        elapsed = time.time() - t_start
        if elapsed >= total:
            break
        remaining = total - elapsed
        n = int(math.ceil(remaining))              # digit currently shown: seconds..1
        frac = remaining - (n - 1)                  # 1.0 at digit start -> 0.0 at digit end
        frac = max(0.0, min(1.0, frac))
        pulse = 1.0 + 0.3 * frac                    # gentle zoom-out from 1.3x to 1.0x
        fade_in = min(1.0, (1.0 - frac) * 6.0 + 0.15)

        canvas = make_fullscreen_canvas(255)
        if cam is not None:
            ret, frame = cam.read()
            if ret:
                last_frame = cv2.flip(frame, 1)
        if last_frame is not None:
            draw_camera_inset(canvas, last_frame)

        cx, cy = SCREEN_W // 2, SCREEN_H // 2
        cv2.putText(canvas, "GET READY", (cx - 95, cy - 130), cv2.FONT_HERSHEY_SIMPLEX,
                    0.8, UI_TEXT_MUTED, 2, cv2.LINE_AA)

        ring_radius, ring_thickness = 95, 6
        cv2.circle(canvas, (cx, cy), ring_radius, (225, 225, 225), ring_thickness, cv2.LINE_AA)
        sweep_deg = int(360 * (1.0 - frac))
        if sweep_deg > 0:
            cv2.ellipse(canvas, (cx, cy), (ring_radius, ring_radius), -90, 0, sweep_deg,
                        UI_ACCENT, ring_thickness, cv2.LINE_AA)

        txt = str(n)
        base_scale = min(SCREEN_W, SCREEN_H) / 380.0
        scale = base_scale * pulse
        thick = max(6, int(base_scale * 5))
        size = text_size(txt, font, scale, thick)
        x = cx - size[0] // 2
        y = cy + size[1] // 2

        overlay = canvas.copy()
        put_text_shadowed(overlay, txt, (x, y), font, scale, UI_TARGET, thick, shadow_offset=4)
        _blend(canvas, overlay, fade_in)

        cv2.imshow(win_name, canvas)
        cv2.waitKey(1)


def draw_plus_icon(canvas, cx, cy, plus_size, color=(255, 255, 255), thickness=2):
    x0 = int(round(cx - plus_size))
    x1 = int(round(cx + plus_size))
    y0 = int(round(cy - plus_size))
    y1 = int(round(cy + plus_size))
    cv2.line(canvas, (x0, cy), (x1, cy), color, thickness, cv2.LINE_AA)
    cv2.line(canvas, (cx, y0), (cx, y1), color, thickness, cv2.LINE_AA)


def draw_target_with_plus(canvas, x, y, radius, show_plus=False):
    """Fixation target: soft accent glow ring, drop shadow, solid dot, crisp crosshair."""
    cx, cy = int(round(x)), int(round(y))
    r = max(1, int(round(radius)))

    # soft translucent glow behind the dot -- makes the pulsing size change
    # read as a gentle "breathing" halo instead of a hard-edged circle.
    glow = canvas.copy()
    cv2.circle(glow, (cx, cy), int(r * 1.9), UI_TARGET_RING, -1, cv2.LINE_AA)
    _blend(canvas, glow, 0.12)
    cv2.circle(canvas, (cx, cy), int(r * 1.4), UI_TARGET_RING, 2, cv2.LINE_AA)

    cv2.circle(canvas, (cx + 2, cy + 3), r, (210, 210, 210), -1, cv2.LINE_AA)  # drop shadow
    cv2.circle(canvas, (cx, cy), r, UI_TARGET, -1, cv2.LINE_AA)               # main dot
    cv2.circle(canvas, (cx, cy), r, (255, 255, 255), 1, cv2.LINE_AA)          # crisp rim

    if show_plus:
        plus_half_len = max(1, int(round(r * 0.45)))
        thickness = max(1, int(round(r * 0.14)))
        draw_plus_icon(canvas, cx, cy, plus_half_len, color=(255, 255, 255), thickness=thickness)
# ----------------------
# Alignment mode
# ----------------------
def mode_alignment():
    cap = cv2.VideoCapture(0)
    win = "Alignment - position your head inside the reference"
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(win, 900, 700)

    align_img = None
    if ALIGN_IMAGE_PATH.exists():
        align_img = cv2.imread(str(ALIGN_IMAGE_PATH), cv2.IMREAD_UNCHANGED)

    ret, frame = cap.read()
    if ret:
        preview = cv2.flip(frame, 1)
        cv2.imshow(win, preview)
        cv2.waitKey(1)
        bring_window_to_front(win)

    print("Alignment mode: Press 's' when aligned and face is green/ok (or 'q' to quit).")

    while True:
        ret, frame = cap.read()
        if not ret:
            break
        h, w = frame.shape[:2]
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = face_mesh.process(rgb)

        present, brightness, bbox = face_present_and_bright(results, frame)
        preview = cv2.flip(frame, 1)
        overlay = preview.copy()
        if align_img is not None:
            scale = 0.6
            aw = int(preview.shape[1] * scale)
            ah = int(align_img.shape[0] * (aw / align_img.shape[1]))
            align_resized = cv2.resize(align_img, (aw, ah), interpolation=cv2.INTER_AREA)
            if align_resized.shape[2] == 4:
                alpha = align_resized[:, :, 3] / 255.0
                for c in range(3):
                    overlay[(preview.shape[0]//2 - ah//2):(preview.shape[0]//2 + ah//2),
                            (preview.shape[1]//2 - aw//2):(preview.shape[1]//2 + aw//2), c] = \
                        (alpha * align_resized[:, :, c] + (1-alpha) * overlay[(preview.shape[0]//2 - ah//2):(preview.shape[0]//2 + ah//2),
                                                                              (preview.shape[1]//2 - aw//2):(preview.shape[1]//2 + aw//2), c])
            else:
                y0 = preview.shape[0]//2 - ah//2
                x0 = preview.shape[1]//2 - aw//2
                overlay[y0:y0+ah, x0:x0+aw] = cv2.addWeighted(align_resized, 0.85, overlay[y0:y0+ah, x0:x0+aw], 0.15, 0)

        outline_color = (0, 255, 0) if present else (0, 0, 255)
        cv2.rectangle(overlay, (5, 5), (preview.shape[1]-5, preview.shape[0]-5), outline_color, 4)
        if present:
            cv2.putText(overlay, "Face detected. Press 's' to start calibration.", (30, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,255,0), 2)
        else:
            cv2.putText(overlay, "Face not found or too dark. Adjust camera/lighting.", (30, 40),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 2)
            if bbox is not None:
                minx, miny, maxx, maxy = bbox
                pw = preview.shape[1]
                mx0 = pw - int(maxx / w * preview.shape[1])
                mx1 = pw - int(minx / w * preview.shape[1])
                my0 = int(miny / h * preview.shape[0])
                my1 = int(maxy / h * preview.shape[0])
                cv2.rectangle(overlay, (mx0, my0), (mx1, my1), (0,0,255), 2)

        cv2.imshow(win, overlay)
        key = cv2.waitKey(1) & 0xFF
        if key in (ord('s'), ord('S')) and present:
            break
        if key == ord('q'):
            break

    cap.release()
    cv2.destroyWindow(win)

# ----------------------
# Calibration flow
# ----------------------
def _wait_until_face_present_or_quit(cap, win, resume_hold=0.7):
    """
    Shared "face lost mid-sampling" pause used by every calibration phase.
    Shows a paused banner + live camera preview until the face reappears,
    then a brief "resuming" confirmation. Returns True if the user quit
    ('q') while paused, False otherwise (including a dead camera read,
    matching the previous behaviour of simply falling through).
    """
    while True:
        ret, frame = cap.read()
        if not ret:
            return False
        canvas = make_fullscreen_canvas(255)
        draw_camera_inset(canvas, cv2.flip(frame, 1), ok=False)
        draw_status_banner(canvas, "Paused - face lost or too dark. Adjust and hold still...", kind="error")
        cv2.imshow(win, canvas)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            return True

        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = face_mesh.process(rgb)
        present, _, _ = face_present_and_bright(results, frame)
        if present:
            ok_canvas = make_fullscreen_canvas(255)
            draw_camera_inset(ok_canvas, cv2.flip(frame, 1), ok=True)
            draw_status_banner(ok_canvas, "Face detected - resuming...", kind="success")
            cv2.imshow(win, ok_canvas)
            cv2.waitKey(1)
            time.sleep(resume_hold)
            return False


def _wait_for_initial_face(cap, win, tx, ty, displayed_radius, start_message):
    """
    Shared "wait for a good face reading before starting to sample at this
    target" gate, used before Phase 1 (grid) and Phase 3 (corners) sampling.
    Returns True if the user quit early.
    """
    while True:
        ret, frame = cap.read()
        if not ret:
            return False
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = face_mesh.process(rgb)
        present, _, _ = face_present_and_bright(results, frame)
        if present:
            canvas = make_fullscreen_canvas(255)
            draw_target_with_plus(canvas, tx, ty, displayed_radius, show_plus=True)
            draw_status_banner(canvas, start_message, kind="success")
            cv2.imshow(win, canvas)
            cv2.waitKey(300)
            return False
        canvas = make_fullscreen_canvas(255)
        draw_camera_inset(canvas, cv2.flip(frame, 1), ok=False)
        draw_status_banner(canvas, "Face not detected or too dark. Adjust camera/lighting.", kind="error")
        cv2.imshow(win, canvas)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            return True
        time.sleep(0.05)


def _collect_samples_at_target(cap, win, target_x, target_y, displayed_radius, count, message_prefix):
    """
    Shared sampling loop used by every calibration phase (grid points,
    edge holds, corners): collects up to `count` (ex, ey, target_x,
    target_y) eye-center samples while pulsing the target and showing a
    progress banner, pausing via _wait_until_face_present_or_quit if the
    face drops out. Returns (new_samples, early_quit, final_displayed_radius).
    """
    samples = 0
    new_samples = []
    sample_target_radius = TARGET_RADIUS * 0.6
    while samples < count:
        ret, frame = cap.read()
        if not ret:
            break
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = face_mesh.process(rgb)
        present, _, _ = face_present_and_bright(results, frame)
        if not present:
            if _wait_until_face_present_or_quit(cap, win):
                return new_samples, True, displayed_radius
            continue

        centers = get_iris_centers(results)
        if centers:
            lx_n, ly_n, rx_n, ry_n = centers
            ex = (lx_n + rx_n) / 2.0
            ey = (ly_n + ry_n) / 2.0
            new_samples.append((ex, ey, target_x, target_y))
            samples += 1

        displayed_radius += (sample_target_radius - displayed_radius) * 0.12

        canvas = make_fullscreen_canvas(255)
        draw_target_with_plus(canvas, target_x, target_y, displayed_radius, show_plus=True)
        draw_status_banner(canvas, f"{message_prefix} {samples}/{count}", progress=(samples, count), kind="info")
        cv2.imshow(win, canvas)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            return new_samples, True, displayed_radius

    return new_samples, False, displayed_radius


def mode_calibration(num_points=16):
    print("Starting calibration. Make sure you're aligned and press Enter to begin.")
    input("Press Enter to start (or Ctrl+C to cancel)...")

    if num_points == 9:
        n = 3
    else:
        n = 4
        num_points = 16

    xs = np.linspace(0.12, 0.88, n)
    ys = np.linspace(0.12, 0.88, n)
    grid_targets = [(int(SCREEN_W * x), int(SCREEN_H * y)) for y in ys for x in xs]

    center = (SCREEN_W // 2, SCREEN_H // 2)
    edge_targets = [
        ("left",  (int(SCREEN_W * 0.05), center[1])),
        ("right", (int(SCREEN_W * 0.95), center[1])),
        ("up",    (center[0], int(SCREEN_H * 0.05))),
        ("down",  (center[0], int(SCREEN_H * 0.95))),
    ]

    corners = [
        (int(SCREEN_W * 0.05), int(SCREEN_H * 0.05)),
        (int(SCREEN_W * 0.95), int(SCREEN_H * 0.05)),
        (int(SCREEN_W * 0.95), int(SCREEN_H * 0.95)),
        (int(SCREEN_W * 0.05), int(SCREEN_H * 0.95)),
    ]

    cap = cv2.VideoCapture(0)
    for _ in range(8):
        cap.read()

    win = "Calibration - look at the red dot"
    cv2.namedWindow(win, cv2.WINDOW_NORMAL)
    cv2.setWindowProperty(win, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.imshow(win, make_fullscreen_canvas(255)); cv2.waitKey(1)
    bring_window_to_front(win)
    hide_system_cursor()

    countdown_on_canvas(win, seconds=3, cam=cap)

    def show_center_message(text, secs=3):
        t0 = time.time()
        font = cv2.FONT_HERSHEY_SIMPLEX
        scale = min(SCREEN_W, SCREEN_H) / 1400.0
        thickness = max(2, int(2.4 * scale))
        max_width = int(SCREEN_W * 0.55)

        words = str(text).split()
        lines, cur = [], ""
        for wd in words:
            test = (cur + " " + wd).strip()
            tw, _ = text_size(test, font, scale, thickness)
            if tw > max_width and cur:
                lines.append(cur)
                cur = wd
            else:
                cur = test
        if cur:
            lines.append(cur)

        line_h = int(52 * scale)
        pad_x, pad_y = 70, 46
        block_h = len(lines) * line_h
        max_line_w = max(text_size(ln, font, scale, thickness)[0] for ln in lines)
        panel_w = max_line_w + pad_x * 2
        panel_h = block_h + pad_y * 2 + 22  # extra room for the progress bar
        x0 = (SCREEN_W - panel_w) // 2
        y0 = (SCREEN_H - panel_h) // 2

        while True:
            elapsed = time.time() - t0
            if elapsed >= secs:
                return False
            ret, frame = cap.read()
            canvas = make_fullscreen_canvas(255)
            if ret:
                draw_camera_inset(canvas, cv2.flip(frame, 1))

            fade = min(1.0, elapsed * 4.0)  # quick fade-in for the card

            card = canvas.copy()
            draw_rounded_rect(card, (x0, y0 + 6), (x0 + panel_w, y0 + panel_h + 6), 24, UI_PANEL_SHADOW, -1)
            _blend(canvas, card, 0.4 * fade)

            card = canvas.copy()
            draw_rounded_rect(card, (x0, y0), (x0 + panel_w, y0 + panel_h), 24, UI_PANEL, -1)
            draw_rounded_rect(card, (x0, y0), (x0 + panel_w, y0 + panel_h), 24, UI_ACCENT, 2)
            ty = y0 + pad_y
            for ln in lines:
                (tw, th) = text_size(ln, font, scale, thickness)
                tx = x0 + (panel_w - tw) // 2
                ty += th
                put_text_shadowed(card, ln, (tx, ty), font, scale, UI_TEXT_DARK, thickness, shadow_offset=1)
                ty += int(line_h - th)
            # thin progress bar along the bottom of the card showing time left
            bar_x0, bar_x1 = x0 + pad_x, x0 + panel_w - pad_x
            bar_y0 = y0 + panel_h - 28
            bar_y1 = y0 + panel_h - 20
            draw_rounded_rect(card, (bar_x0, bar_y0), (bar_x1, bar_y1), 4, (228, 228, 228), -1)
            frac = max(0.0, min(1.0, elapsed / secs))
            fill_x1 = bar_x0 + int((bar_x1 - bar_x0) * frac)
            if fill_x1 > bar_x0:
                draw_rounded_rect(card, (bar_x0, bar_y0), (fill_x1, bar_y1), 4, UI_ACCENT, -1)
            _blend(canvas, card, fade)

            cv2.imshow(win, canvas)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                return True

    if show_center_message("Follow the dots", secs=3):
        cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

    collected = []
    drag_record_list = []

    def draw_center_arrow(canvas, tx, ty):
        cx, cy = center
        vecx = tx - cx; vecy = ty - cy
        norm = math.hypot(vecx, vecy)
        if norm < 1: norm = 1.0
        length = min(300, int(norm * 0.6) if norm > 0 else 100)
        ux = int(round(cx + (vecx / norm) * length))
        uy = int(round(cy + (vecy / norm) * length))
        cv2.arrowedLine(canvas, (cx, cy), (ux, uy), (0,0,255), 8, tipLength=0.25)

    def animate_transition_and_maybe_sample(sx0, sy0, sx1, sy1,
                                           record_while=False, max_samples=0, instruction=None,
                                           drag_mode=False, edge_name=None):
        rec = []
        t_start = time.time()
        samples_taken = 0
        duration = DRAG_TRANSITION_SEC if drag_mode else TRANSITION_SEC
        peak_rr = 0.0
        while True:
            elapsed = time.time() - t_start
            alpha = min(1.0, elapsed / duration)
            px = int(round(sx0 + alpha * (sx1 - sx0)))
            py = int(round(sy0 + alpha * (sy1 - sy0)))
            canvas = make_fullscreen_canvas(255)
            rr = TARGET_RADIUS * (0.8 + 0.4 * (1 - abs(0.5 - alpha)))
            if rr > peak_rr:
                peak_rr = rr
            rr_i = int(round(rr))
            draw_target_with_plus(canvas, px, py, rr_i, show_plus=True)
            if instruction:
                draw_status_banner(canvas, instruction, kind="info")
            cv2.imshow(win, canvas)

            ret, frame = cap.read()
            if not ret:
                break
            rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            results = face_mesh.process(rgb)

            if record_while and samples_taken < max_samples:
                centers = get_iris_centers(results)
                if centers:
                    lx_n, ly_n, rx_n, ry_n = centers
                    ex = (lx_n + rx_n) / 2.0
                    ey = (ly_n + ry_n) / 2.0
                    ts = time.time()
                    rec.append({"timestamp": ts, "ex": float(ex), "ey": float(ey),
                                "px": int(px), "py": int(py), "phase": "drag", "edge": edge_name})
                    samples_taken += 1

            if alpha >= 1.0:
                rr_final = TARGET_RADIUS
                final_canvas = make_fullscreen_canvas(255)
                draw_target_with_plus(final_canvas, sx1, sy1, int(round(rr_final)), show_plus=True)
                cv2.imshow(win, final_canvas)
                cv2.waitKey(1)
                time.sleep(0.12)
                return rec, False, float(rr_final), float(peak_rr)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                return rec, True, float(TARGET_RADIUS * 0.6), float(peak_rr)

    # Phase 1: grid sampling
    for i, (tx, ty) in enumerate(grid_targets):
        if i == 0:
            cur_x, cur_y = center
        else:
            cur_x, cur_y = grid_targets[i-1]

        rec, early_quit, end_radius, peak_radius = animate_transition_and_maybe_sample(cur_x, cur_y, tx, ty, record_while=False)
        if early_quit:
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

        displayed_radius = float(peak_radius if peak_radius and peak_radius > 0 else end_radius)
        if _wait_for_initial_face(cap, win, tx, ty, displayed_radius, "Starting sample..."):
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

        new_samples, early_quit, displayed_radius = _collect_samples_at_target(
            cap, win, tx, ty, displayed_radius, SAMPLES_PER_POINT, "Collecting")
        collected.extend(new_samples)
        if early_quit:
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

    # Phase 2: drags + edge sampling
    phase2_instruction = "Drag Head with the pointer."
    if show_center_message(phase2_instruction, secs=3):
        cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

    for edge_name, (etx, ety) in edge_targets:
        rec, early, end_radius, peak_radius = animate_transition_and_maybe_sample(center[0], center[1], etx, ety,
                                                                                   record_while=True, max_samples=SAMPLES_PER_DRAG,
                                                                                   instruction=None, drag_mode=True, edge_name=edge_name)
        if early:
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

        for d in rec:
            collected.append((d["ex"], d["ey"], d["px"], d["py"]))
        drag_record_list.extend(rec)

        displayed_radius = float(peak_radius if peak_radius and peak_radius > 0 else end_radius)
        new_samples, early_quit, displayed_radius = _collect_samples_at_target(
            cap, win, etx, ety, displayed_radius, SAMPLES_PER_POINT, "Edge-sampling")
        collected.extend(new_samples)
        if early_quit:
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

        return_duration = 0.6
        t_start = time.time()
        while True:
            elapsed = time.time() - t_start
            alpha = min(1.0, elapsed / return_duration)
            px = int(round(etx + alpha * (center[0] - etx)))
            py = int(round(ety + alpha * (center[1] - ety)))
            canvas = make_fullscreen_canvas(255)
            rr = TARGET_RADIUS * (0.8 + 0.4 * (1 - abs(0.5 - alpha)))
            rr_i = int(round(rr))
            draw_target_with_plus(canvas, px, py, rr_i, show_plus=True)
            cv2.imshow(win, canvas)

            ret, frame = cap.read()
            if not ret:
                break

            if alpha >= 1.0:
                break
            if cv2.waitKey(1) & 0xFF == ord('q'):
                cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return
            time.sleep(0.01)

    try:
        existing = []
        if OUT_DRAG_JSON.exists():
            with open(OUT_DRAG_JSON, "r") as f:
                try: existing = json.load(f)
                except Exception: existing = []
        existing.extend(drag_record_list)
        with open(OUT_DRAG_JSON, "w") as f:
            json.dump(existing, f, indent=2)
    except Exception as e:
        print("Warning: failed to save drag JSON:", e)

    # Phase 3: corners sampling
    for i, (tx, ty) in enumerate(corners):
        if i == 0:
            start_x, start_y = center
        else:
            start_x, start_y = corners[i-1]
        rec, early_quit, end_radius, peak_radius = animate_transition_and_maybe_sample(start_x, start_y, tx, ty, record_while=False)
        if early_quit:
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

        displayed_radius = float(peak_radius if peak_radius and peak_radius > 0 else end_radius)
        if _wait_for_initial_face(cap, win, tx, ty, displayed_radius, "Starting corner sample..."):
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

        new_samples, early_quit, displayed_radius = _collect_samples_at_target(
            cap, win, tx, ty, displayed_radius, SAMPLES_PER_POINT, "Collecting corner")
        collected.extend(new_samples)
        if early_quit:
            cap.release(); show_system_cursor(); cv2.destroyAllWindows(); return

    cap.release()
    show_system_cursor()
    cv2.destroyAllWindows()

    if len(collected) < 6:
        print("Not enough samples collected.")
        return
    wx, wy = fit_poly_mapping(collected)

    pred_x = np.array([predict_poly(wx, wy, ex, ey)[0] for ex, ey, _, _ in collected])
    pred_y = np.array([predict_poly(wx, wy, ex, ey)[1] for ex, ey, _, _ in collected])
    true_x = np.array([r[2] for r in collected])
    true_y = np.array([r[3] for r in collected])

    invert_x = invert_y = False
    try:
        cx, _ = pearsonr(pred_x, true_x)
        cy, _ = pearsonr(pred_y, true_y)
        if cx < 0: invert_x = True
        if cy < 0: invert_y = True
    except Exception:
        pass

    print("\n" + "="*40)
    user_node_name = input("Enter a name for this calibration profile (e.g. user_daylight) [Or press Enter for 'default']: ").strip()
    if not user_node_name:
        user_node_name = "default"

    save_mapping(wx, wy, invert_x=invert_x, invert_y=invert_y, profile_name=user_node_name)

    errors = np.sqrt((pred_x - true_x) ** 2 + (pred_y - true_y) ** 2)
    print("Calibration finished. Mean error: {:.1f}px, Median: {:.1f}px".format(errors.mean(), np.median(errors)))
# ----------------------
# Cursor control
# ----------------------
class PreviewWindow:
    """
    Runs the OpenCV preview window (cv2.imshow / cv2.waitKey) on its OWN thread.

    WHY THIS FIXES THE FREEZE:
    On Windows, the preview window created by cv2.imshow is a normal, real
    OS window with a title bar. When the pinch gesture calls
    pyautogui.mouseDown() and then keeps moving the (real, system) mouse
    cursor while the button is held down, and that cursor happens to be
    over the preview window (its title bar in particular), Windows itself
    decides "the user is dragging this window" and enters its own native,
    blocking move/drag loop for that window (this is standard OS behaviour,
    not a bug in mediapipe/pyautogui).

    That native loop only returns control once the mouse button is
    released. The problem was that this window's message pump
    (cv2.waitKey) was being called from the SAME thread that also reads
    the camera, runs Mediapipe, evaluates gestures, and drives the OS
    cursor via pyautogui. So the moment Windows entered that native drag
    loop, the whole thread — and therefore the whole script, including all
    mouse control — froze solid until you clicked elsewhere and forced
    Windows to release its internal mouse capture.

    By giving the preview window its own dedicated thread, a native
    window-drag on the preview window can only block THAT thread. The
    camera/gesture/mouse-control loop keeps running on the main thread
    completely unaffected, so the OS cursor, clicks and the pinch-drag
    keep working normally, and you can freely grab the preview window's
    title bar and move it around without freezing anything.
    """

    def __init__(self, win_name, width=480, height=360):
        self.win_name = win_name
        self.width = width
        self.height = height
        self._lock = threading.Lock()
        self._frame = None
        self._stop_event = threading.Event()
        self._thread = threading.Thread(target=self._run, daemon=True)

    def start(self):
        self._thread.start()

    def update(self, frame):
        # Store a copy so the main thread is free to keep mutating/reusing
        # its own frame buffer without racing the display thread.
        with self._lock:
            self._frame = frame.copy()

    def _run(self):
        cv2.namedWindow(self.win_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(self.win_name, self.width, self.height)
        while not self._stop_event.is_set():
            with self._lock:
                frame = None if self._frame is None else self._frame
            if frame is not None:
                cv2.imshow(self.win_name, frame)
            # This waitKey() is the Win32 message pump for THIS window only.
            # If Windows enters a native move/drag modal loop here, it can
            # only stall this thread — never the gesture/mouse-control loop.
            cv2.waitKey(15)
        cv2.destroyWindow(self.win_name)

    def stop(self):
        self._stop_event.set()
        self._thread.join(timeout=2)


def mode_cursor_control():
    # Because we cv2.flip the frame, MediaPipe's "Left Eye" indices actually land on the subject's physical Right Eye.
    # To keep the code sane, we map the names accurately to your PHYSICAL eyes so it's not confusing anymore.
    PHYSICAL_LEFT_EYE = [33, 159, 145, 133]  
    PHYSICAL_RIGHT_EYE = [362, 386, 374, 263]
    
    mapping = load_mapping()
    if mapping is None: 
        print("\n[ERROR] No active tracking profile found! Please run Calibration (2) or Load a profile node first.")
        return
    
    wx = np.array(mapping["wx"]); wy = np.array(mapping["wy"])
    invert_x = True 
    
    cap = cv2.VideoCapture(0)
    ema_x, ema_y = None, None
    last_click_time = 0
    
    is_left_winked = False
    is_right_winked = False
    
    # State tracking variables for clicking and dragging
    l_wink_start_ms = None
    double_click_triggered = False
    is_dragging = False

    # EAR smoothing buffers and hysteresis counters
    # (prevent corner/edge wink-hold interruptions)
    l_ear_smooth = None
    r_ear_smooth = None
    l_open_count = 0   # consecutive frames left eye read as "open"
    r_open_count = 0   # consecutive frames right eye read as "open"

    # Pinch smoothing + release hysteresis
    # (prevent brief detection dropouts from releasing the hold)
    norm_dist_smooth = None
    pinch_release_count = 0  # consecutive non-pinch frames; release only after threshold

    profile_label = mapping.get("profile_name", "default")
    print(f"Cursor Control Active using Node Profile: '{profile_label}'. Press 'CTRL+Q' anywhere to exit.")

    # The preview window now lives on its own thread — see PreviewWindow
    # docstring above for why this is what stops the freeze.
    preview = PreviewWindow("Wink Debugging (Press CTRL+Q to stop)")
    preview.start()

    try:
        while True:
            if keyboard.is_pressed('ctrl+q'):
                print("\nUniversal exit triggered. Closing...")
                break

            ret, frame = cap.read()
            if not ret: break
            
            frame = cv2.flip(frame, 1)
            h, w, _ = frame.shape
            rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            
            # Process both Face Mesh (eyes) and Hands
            results_face = face_mesh.process(rgb)
            results_hands = hands.process(rgb)
            
            status_text = "Status: Monitoring"
            status_color = (0, 255, 0)

            # 1. Pinch to Hold logic — smoothed distance + release hysteresis
            # pinch_seen: True if this frame's hand reading shows fingers together.
            # The actual mouseDown/Up is governed by pinch_release_count so that
            # brief detection dropouts or jitter never cut the hold short.
            pinch_seen = False
            if results_hands.multi_hand_landmarks:
                for hand_landmarks in results_hands.multi_hand_landmarks:
                    mp_drawing.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)
                    
                    thumb_tip = hand_landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
                    index_tip = hand_landmarks.landmark[mp_hands.HandLandmark.INDEX_FINGER_TIP]
                    
                    # Raw normalised finger distance
                    norm_dist = math.hypot(thumb_tip.x - index_tip.x, thumb_tip.y - index_tip.y)

                    # ── Distance smoothing ────────────────────────────────────
                    # Blend with the previous frame so a single jittery reading
                    # cannot spike the distance above the release threshold.
                    if norm_dist_smooth is None:
                        norm_dist_smooth = norm_dist
                    else:
                        norm_dist_smooth = (PINCH_DIST_EMA_ALPHA * norm_dist
                                           + (1 - PINCH_DIST_EMA_ALPHA) * norm_dist_smooth)

                    # Engage threshold tighter than release threshold (hysteresis).
                    # Release threshold widened to 0.12 to give fingers breathing room.
                    if not is_dragging and norm_dist_smooth < 0.05:
                        pinch_seen = True
                    elif is_dragging and norm_dist_smooth < 0.12:
                        pinch_seen = True

                    if pinch_seen:
                        tx, ty = int(thumb_tip.x * w), int(thumb_tip.y * h)
                        ix, iy = int(index_tip.x * w), int(index_tip.y * h)
                        cv2.circle(frame, ((tx + ix) // 2, (ty + iy) // 2), 12, (0, 255, 0), -1)

            current_ms = time.time() * 1000

            # ── Pinch state machine ───────────────────────────────────────────
            if pinch_seen:
                pinch_release_count = 0          # fingers together → reset release counter
                if not is_dragging:
                    pyautogui.mouseDown(button='left')
                    is_dragging = True
                    last_click_time = current_ms
                # if already dragging, do nothing — hold is maintained
            else:
                if is_dragging:
                    # Fingers apart (or hand lost) — count consecutive non-pinch frames.
                    # Only release after PINCH_RELEASE_FRAMES so brief dropouts are ignored.
                    pinch_release_count += 1
                    if pinch_release_count >= PINCH_RELEASE_FRAMES:
                        pyautogui.mouseUp(button='left')
                        is_dragging = False
                        last_click_time = current_ms
                        pinch_release_count = 0
                        norm_dist_smooth = None  # reset smoothing for next pinch

            # Set Drag UI Status (Prioritized over regular clicks)
            if is_dragging:
                status_text = "DRAGGING (Pinch Active)"
                status_color = (0, 255, 0)

            # 2. Eye Processing & Cursor Movement Logic
            if results_face.multi_face_landmarks:
                landmarks = results_face.multi_face_landmarks[0].landmark
                
                for idx in (PHYSICAL_LEFT_EYE + PHYSICAL_RIGHT_EYE):
                    pt = landmarks[idx]
                    cv2.circle(frame, (int(pt.x * w), int(pt.y * h)), 2, (0, 0, 255), -1)

                centers = get_iris_centers(results_face)
                if centers:
                    lx_n, ly_n, rx_n, ry_n = centers
                    px, py = predict_poly(wx, wy, (lx_n + rx_n) / 2.0, (ly_n + ry_n) / 2.0)
                    if invert_x: px = SCREEN_W - px
                    px = float(np.clip(px, 0, SCREEN_W-1))
                    py = float(np.clip(py, 0, SCREEN_H-1))

                    if ema_x is None: ema_x, ema_y = px, py
                    else:
                        dist = math.hypot(px - ema_x, py - ema_y)
                        alpha = EMA_ALPHA + min(0.8, (dist / max(SCREEN_W, SCREEN_H)) * 3.0)
                        ema_x = alpha * px + (1 - alpha) * ema_x
                        ema_y = alpha * py + (1 - alpha) * ema_y
                    pyautogui.moveTo(int(ema_x), int(ema_y), _pause=False)

                # Now accurately mapped to your physical eyes
                l_ear_raw = calculate_ear(landmarks, PHYSICAL_LEFT_EYE)
                r_ear_raw = calculate_ear(landmarks, PHYSICAL_RIGHT_EYE)

                # ── EAR smoothing ──────────────────────────────────────────────
                # Blend raw values with previous smoothed value each frame.
                # This kills single-frame landmark spikes that would otherwise
                # interrupt a hold at screen corners / edges.
                if l_ear_smooth is None:
                    l_ear_smooth = l_ear_raw
                    r_ear_smooth = r_ear_raw
                else:
                    l_ear_smooth = EAR_EMA_ALPHA * l_ear_raw + (1 - EAR_EMA_ALPHA) * l_ear_smooth
                    r_ear_smooth = EAR_EMA_ALPHA * r_ear_raw + (1 - EAR_EMA_ALPHA) * r_ear_smooth

                l_ear = l_ear_smooth
                r_ear = r_ear_smooth

                l_diff = r_ear - l_ear
                r_diff = l_ear - r_ear
                
                # Left Wink Logic (Triggers Left Click / Double Click on Long Hold)
                # Thresholds slightly relaxed (0.18→0.20, diff 0.07→0.05) to
                # compensate for reduced EAR range when gazing at corners.
                if l_ear < 0.20 and l_diff > 0.05:
                    l_open_count = 0  # eye is closed — reset the open-frame counter
                    if l_wink_start_ms is None:
                        l_wink_start_ms = current_ms
                        double_click_triggered = False
                    
                    hold_duration = current_ms - l_wink_start_ms
                    
                    if hold_duration >= DOUBLE_CLICK_HOLD_MS:
                        if not is_dragging:  # Ensure we don't disrupt dragging mode
                            status_text = "DOUBLE CLICK (Left Eye)"
                            status_color = (0, 255, 255)
                            if not double_click_triggered:
                                pyautogui.doubleClick(button='left')
                                double_click_triggered = True
                                last_click_time = current_ms
                    else:
                        if not is_dragging:
                            status_text = "LEFT CLICK (Left Eye) - Holding..."
                            status_color = (0, 0, 255)
                        if not is_left_winked and (current_ms - last_click_time) > WINK_COOLDOWN_MS:
                            if not is_dragging:
                                pyautogui.click(button='left')
                            is_left_winked = True
                            last_click_time = current_ms
                elif l_ear > 0.22:
                    # ── Hysteretic reset ───────────────────────────────────────
                    # Require WINK_RESET_FRAMES consecutive "open" frames before
                    # clearing the wink state.  A single noisy frame at a corner
                    # no longer kills the hold timer.
                    l_open_count += 1
                    if l_open_count >= WINK_RESET_FRAMES:
                        l_wink_start_ms = None
                        is_left_winked = False
                        double_click_triggered = False

                # Right Wink Logic (Triggers Right Click)
                if r_ear < 0.20 and r_diff > 0.05:
                    r_open_count = 0
                    if not is_dragging:
                        status_text = "RIGHT CLICK (Right Eye)"
                        status_color = (255, 0, 0)
                    if not is_right_winked and (current_ms - last_click_time) > WINK_COOLDOWN_MS:
                        if not is_dragging:
                            pyautogui.click(button='right')
                        is_right_winked = True
                        last_click_time = current_ms
                elif r_ear > 0.22:
                    r_open_count += 1
                    if r_open_count >= WINK_RESET_FRAMES:
                        is_right_winked = False

                cv2.putText(frame, f"L: {l_ear:.2f} R: {r_ear:.2f}  (smoothed)", (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

            cv2.putText(frame, f"Profile: {profile_label}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
            cv2.putText(frame, status_text, (10, h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, status_color, 2)

            # Hand the finished frame off to the display thread instead of
            # calling cv2.imshow/cv2.waitKey directly on this thread.
            preview.update(frame)

    finally:
        # Make sure a held-down drag never gets "stuck" if we exit mid-pinch.
        if is_dragging:
            pyautogui.mouseUp(button='left')
        preview.stop()
        cap.release()

# ----------------------
# Main
# ----------------------

if __name__ == "__main__":
    while True:
        print("""
    GAZE PROTOTYPE MENU
    1 -> Alignment (camera + alignment reference)
    2 -> Calibration (smooth 4x4 + slow drags + edge sampling + corners)
    3 -> Cursor control (overlay + OS cursor moves)
    4 -> Profile Management (Load saved calibration profiles)
    q -> Quit
        """)
        cmd = input("Enter mode: ").strip().lower()
        if cmd == "1":
            mode_alignment()
        elif cmd == "2":
            mode_calibration(num_points=16)
        elif cmd == "3":
            mode_cursor_control()
        elif cmd == "4":
            interactive_profile_menu()
        elif cmd == "q":
            break
        else:
            print("Invalid. Choose 1, 2, 3, 4 or q.")